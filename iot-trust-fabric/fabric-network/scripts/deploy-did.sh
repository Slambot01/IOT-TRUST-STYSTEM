#!/bin/bash
# =============================================================================
# deploy-did.sh — Deploy the DID Registry Chaincode
# =============================================================================
# This script packages, installs, approves, and commits the did-registry
# chaincode to the iot-channel. Then runs a test invocation.
#
# Prerequisites:
...
# Configuration
# ---------------------------------------------------------------------------
CC_NAME="did-registry"
CC_VERSION="1.0"
CC_SEQUENCE=1
CC_LABEL="${CC_NAME}_${CC_VERSION}"
CC_SRC_PATH="/opt/gopath/src/github.com/hyperledger/fabric-samples/chaincode/did-registry"
...
# ---------------------------------------------------------------------------
# Step 1: Install npm dependencies inside the chaincode directory
# ---------------------------------------------------------------------------
...
docker exec cli bash -c "cd ${CC_SRC_PATH} && npm install --production 2>&1" || {
    ...
    CHAINCODE_LOCAL_DIR="$(cd "${SCRIPT_DIR}/../../chaincode/did-registry" && pwd)"
...
# ---------------------------------------------------------------------------
# Step 8: Test invocation — Register a device DID
# ---------------------------------------------------------------------------
info "Step 8: Running test invocation..."
...
docker exec cli peer chaincode invoke \
    ...
    --peerAddresses ${PEER_ADDRESS} \
    --tlsRootCertFiles ${PEER_TLS_ROOTCERT} \
    -c '{"function":"registerDID","Args":["device001","testPublicKey","http://localhost:3000"]}'

if [ $? -eq 0 ]; then
    success "Test invocation successful: device001 registered."
else
    warn "Test invocation failed. The chaincode container may still be starting."
...
echo ""
echo -e "${GREEN}========================================${NC}"
echo -e "${GREEN} DID Registry Chaincode Deployed!       ${NC}"
echo -e "${GREEN}========================================${NC}"
echo ""
...
echo ""
info "Next step: Deploy trust-score chaincode with ./scripts/deploy-trust.sh"
echo ""
